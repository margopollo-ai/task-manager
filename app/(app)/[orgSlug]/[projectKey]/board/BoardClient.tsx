            ...
            {sections.map((section) => (
              <div key={section.id}>
                <div className="flex items-center gap-2 mb-3">
                  <h2 className={`font-extrabold text-lg ${section.completed ? "text-gray-400 line-through" : "text-fuchsia-600"}`}>{section.title}</h2>
                  {section.goalKey && (
                    <span className={`text-xs font-mono px-1.5 py-0.5 rounded border ${section.completed ? "bg-gray-50 text-gray-400 border-gray-200" : "bg-fuchsia-50 text-fuchsia-500 border-fuchsia-200"}`}>
                      {section.goalKey}
                    </span>
                  )}
                  {section.completed && (
